...
注意：为了保证“观看历史”和“最大播放进度”不丢，Ruru 当前没有因为 Redis 命中就完全跳过 MySQL。Redis 这一阶段主要负责挡住重复计数和减少写计数压力；feed/detail 缓存和后续互动计数缓存会继续降低读取压力。

## 首页和详情短缓存

Ruru 当前对首页 feed 和帖子详情使用短 TTL 缓存，TTL 为 2 分钟。

当前缓存 key：

```text
ruru:feed:all:0
ruru:post:detail:{postId}
```

缓存内容只保存公开基础数据，不把“当前用户是否点赞、是否投猪、是否收藏”这类个性化字段当成长期可信数据。登录用户读取缓存后，后端会再根据当前用户重新叠加这些状态。

这么做的原因是：

- 游客和登录用户可以共用一份公开内容缓存，减少 MySQL 查询。
- 点赞、投猪、收藏这些个人状态必须实时，否则用户刚点完按钮会看到旧状态。
- R2 读取 URL 是有过期时间的，缓存 TTL 不能太长，避免前端拿到已经过期的媒体链接。

缓存失效规则：

- 发布公开帖子后，清理首页 feed 缓存。
- 更新、删除、隐藏、恢复、审核帖子后，清理首页 feed 和对应详情缓存。
- 评论、弹幕、点赞、投猪、收藏、播放计数变化后，清理首页 feed 和对应详情缓存。
- 管理员删除评论或弹幕后，也清理相关帖子缓存。

这不是最省 Redis 的方案，但非常稳：数据不会长时间错，代码也容易理解。等后续阶段接入“互动计数缓存”后，点赞数、投猪数、收藏数、播放数会进一步从 Redis 读取，首页会更轻。

## 热点计数缓存

Ruru 当前把帖子热点计数单独放进 Redis，TTL 为 6 小时。

当前计数 key：

```text
ruru:counter:post:{postId}
```
...
- 点赞、取消点赞、投猪、收藏、取消收藏、有效播放计数成功写入 MySQL 后，会刷新对应帖子的 Redis 计数。
- 首页 feed 或帖子详情命中页面缓存时，会再读取热点计数并覆盖页面缓存里的旧数字。
- 页面缓存没命中时，本来就会查 MySQL 最新数据，不额外读取计数缓存。
- 计数缓存丢失时，从 MySQL 回源，并重新写回 Redis。
- Redis 不可用时，仍然返回 MySQL 或页面缓存中的数据，业务不中断。

这里没有做“Redis 先加、以后批量落库”的激进方案，因为当前阶段更重要的是数据可信和容易排查。等后续接入消息队列后，再考虑异步批量落库。
...
2. 首页 feed 和帖子详情短缓存：2 分钟 TTL，公开内容缓存，登录用户状态实时叠加。
3. 点赞、收藏、投币、播放等热点计数缓存：写库成功后刷新 Redis，页面缓存命中时覆盖旧计数。
4. Redis ZSet 榜单。
