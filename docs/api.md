...
为了防止刷登录、刷注册、刷上传、刷评论、刷弹幕、刷点赞、刷投币、刷收藏和刷播放上报，后端对部分高频写接口做了 Redis 限流。

被限流时返回 HTTP `429`，响应体仍然使用统一格式。

当前限流策略偏保守，正常使用基本不会触发；如果 Redis 暂时不可用，当前阶段会先放行请求并记录日志，避免 Redis 成为单点故障。
